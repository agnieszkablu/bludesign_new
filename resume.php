<?php get_header(); ?>
	<div class="container">
		<div id="primary" class="content-area">
			<main id="main" class="site-main" role="main">
				<article class="hero-header rellax" data-rellax-speed="-7">
					<h1 class="hero-txt">Résumé</h1>
					<figure class="hero-cv-img">
						<img src="dist/img/aga2.png" alt="Agnieszka Gorz profile photo" >
					</figure>
				</article>
				<article class="entry-content">
					<div class="cv-header">
						<h2>Agnieszka Górz</h2>
						<h3>Freelance webdeveloper</h3>
						<p>Hey! I am Agnieszka Górz, I am a freelance webdeveloper, self-tought and self-employed, and looking for chance to enhance my skills and boost my progress.   </p>
					</div>
					<div class="cv-timeline">
						<ul>
							<li><span class="icon-clipboard"></span>High school <br>2005 – 2008</li>
							<li><span class="icon-university"></span>Bachelors <br>in Business Management <br> 2008 - 2011</li>
							<li><span class="icon-graduation-cap"></span>Masters <br>in Financial Management <br> 2011 - 2013</li>
						</ul>
					</div>
					<div class="cv-content">
						<div class="row">
							<div class="cv-work col-6">
								<h4 class="cv-block-heading">Work Experience</h4>
								<ul>
									<li>
										Freelance webdeveloper [ <span class="lighter">June 2015 - Present</span> ] <br>
										<span class="lighter">Self-employed</span>
									</li>
									<li>
										Manager at Simple-CP boutique [ <span class="lighter">Nov 2013 - Feb 2015</span> ]<br>
										<span class="lighter">Simple Sp. z o.o., Trzy Korony, Nowy Sącz</span>
									</li>
									<li>
										Sales specialist  [ <span class="lighter">June 2011 - nov 2012</span> ]<br>
										<span class="lighter">P.U.H. Auto-Complex Sp. z o.o., Nowy Sącz</span>
									</li>
								</ul>
							</div>
							<div class="cv-infos col-6">
								<h4 class="cv-block-heading">Personal Info</h4>
								<table>
									<tr>
										<th>Name</th>
										<td>Agnieszka Górz</td>
									</tr>
									<tr>
										<th>BirthDay</th>
										<td>01 July 1989</td>
									</tr>
									<tr>
										<th>E-mail</th>
										<td><a href="mailto:agnieszka.gorz@outlook.com">agnieszka.gorz@outlook.com</a></td>
									</tr>
									<tr>
										<th>Address</th>
										<td>Gołkowice Dolne 16, 33-388 Gołkowice, Poland</td>
									</tr>
								</table>
							</div>
						</div>
						<div class="row">
							<div class="cv-skill1 col-3">
								<h4 class="cv-block-heading">Proficiency Skill</h4>
								<table>
									<tr>
										<th>Twitter Bootstrap</th>
										<td class="skill-bullet" data-skillSet="3"></td>
									</tr>
									<tr>
										<th>Photoshop</th>
										<td class="skill-bullet" data-skillSet="2"></td>
									</tr>
									<tr>
										<th>Wordpress</th>
										<td class="skill-bullet" data-skillSet="3"></td>
									</tr>
									<tr>
										<th>Woocommerce</th>
										<td class="skill-bullet" data-skillSet="2"></td>
									</tr>
								<tr>
										<th>Git</th>
										<td class="skill-bullet" data-skillSet="2"></td>
									</tr>
									<tr>
										<th>Grunt</th>
										<td class="skill-bullet" data-skillSet="1"></td>
									</tr>
									<tr>
										<th>Yeoman</th>
										<td class="skill-bullet" data-skillSet="1"></td>
									</tr>

								</table>
							</div>
							<div class="cv-skill2 col-3">
								<h4 class="cv-block-heading">Professional Skill</h4>
								<table>
									<tr>
										<th>Html5</th>
										<td class="skill-bullet" data-skillSet="4"></td>
									</tr>
									<tr>
										<th>Css3</th>
										<td class="skill-bullet" data-skillSet="4"></td>
									</tr>
									<tr>
										<th>Sass</th>
										<td class="skill-bullet" data-skillSet="3"></td>
									</tr>
									<tr>
										<th>JavaScript</th>
										<td class="skill-bullet" data-skillSet="1"></td>
									</tr>
									<tr>
										<th>jQuery</th>
										<td class="skill-bullet" data-skillSet="1"></td>
									</tr>
									<tr>
										<th>PHP</th>
										<td class="skill-bullet" data-skillSet="1"></td>
									</tr>
								</table>
							</div>
							<div class="cv-skill3 col-3">
								<h4 class="cv-block-heading">Personal Skill</h4>
								<table>
									<tr>
										<th>TeamWork</th>
										<td class="skill-bullet" data-skillSet="4"></td>
									</tr>
									<tr>
										<th>Communication skills</th>
										<td class="skill-bullet" data-skillSet="5"></td>
									</tr>
									<tr>
										<th>English</th>
										<td><span class="eng-lvl"> B2 level </span></td>
									</tr>
								</table>
							</div>
							<div class="">*These subjective values are visual representation of Competencies Proficiency Scale which is organized in five steps; from “Fundamental Awareness” to “Expert”.</div>
						</div> <!-- .row -->
						<div class="cv-hobby">
							<h4 class="cv-block-heading">Passions &amp; Hobbies</h4>
							<ul>
								<li><span class="icon-coffee"></span> Coffee</li>
								<li><span class="icon-book"></span> Reading</li>
								<li><span class="icon-bicycle"></span> Sport</li>
								<li><span class="icon-globe"></span> Traveling</li>
								<li><span class="icon-headphones"></span> Music</li>
								<li><span class="icon-gamepad"></span>Gaming</li>
							</ul>
						</div>
						<div class="cv-social">
							<h4 class="cv-block-heading">Contact</h4>
							<ul>
								<li>
									<a href="https://github.com/agnieszkablu"><span class="icon-git"></span>GitHub</a>
								</li>
								<li>
									<a href="www.linkedin.com/in/agnieszkagorz"><span class="icon-linkedin"></span>Linkedin</a>
								</li>
								<li>
									<a href="mailto:agnieszka.gorz@outlook.com"><span class="icon-envelope"></span>agnieszka.gorz@outlook.com</a>
								</li>
							</ul>
						</div>
					</div><!-- .cv-content -->
				</article><!-- .enry-content -->
			</main><!-- #main -->
		</div><!-- #primary -->
	</div><!-- .container -->
<?php get_footer(); ?>